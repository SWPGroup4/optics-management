...
public class VNPayService {
    ...
    public String createPaymentUrl(Payment payment, String returnUrlBase) {
        ...
        while (itr.hasNext()) {
            ...
            if ((fieldValue != null) && (fieldValue.length() > 0)) {
                // Cả hashData và query đều cần URL encode theo chuẩn VNPay
                try {
    ...
    public int orderReturn(HttpServletRequest request) {
        ...
        if (signValue.equals(vnp_SecureHash)) {
            return "00".equals(request.getParameter("vnp_TransactionStatus")) ? 1 : 0; // 1: Thành công, 0: Thất bại
        } else {
            return -1; // Sai mã hash
        }
    }
}